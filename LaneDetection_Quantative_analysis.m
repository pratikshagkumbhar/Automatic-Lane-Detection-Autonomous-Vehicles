function LaneDetection_Quantative_analysis
...
% ---------- TOP ROW ----------
ax1 = axes('Parent',fig,'Position',[0.05 0.72 0.27 0.22]); title(ax1,'Fig3.1 Original','Color','white');
ax2 = axes('Parent',fig,'Position',[0.37 0.72 0.27 0.22]); title(ax2,'Fig3.2 Gray','Color','white');
ax3 = axes('Parent',fig,'Position',[0.69 0.72 0.27 0.22]); title(ax3,'Fig3.3 Gaussian','Color','white');

% ---------- MIDDLE ROW ----------
ax4 = axes('Parent',fig,'Position',[0.05 0.42 0.27 0.22]); title(ax4,'Fig3.4 CLAHE','Color','white');
ax5 = axes('Parent',fig,'Position',[0.37 0.42 0.27 0.22]); title(ax5,'Fig3.5 Median','Color','white');
ax6 = axes('Parent',fig,'Position',[0.69 0.42 0.27 0.22]); title(ax6,'Fig3.6 Edges','Color','white');

% ---------- BOTTOM ROW ----------
ax7 = axes('Parent',fig,'Position',[0.05 0.12 0.27 0.22]); title(ax7,'Fig3.7 ROI','Color','white');
ax8 = axes('Parent',fig,'Position',[0.37 0.12 0.27 0.22]); title(ax8,'Fig3.8 Hough','Color','white');
ax9 = axes('Parent',fig,'Position',[0.69 0.12 0.27 0.22]); title(ax9,'Fig3.9 Final','Color','white');
...
%% Start Detection
    function startDetection(~,~)
        ...
        detectedFrames = 0;      % Successfully detected lane frames
        undetectedFrames = 0;    % Failed detection frames
        tic; % Start timer
        ...
        while hasFrame(VideoFile)
            ...
            %% Preprocessing
                ...
                title(ax1,'Fig3.1.Original','Color','white');
            ...
            if mod(frameCount,2)==0
                ...
                title(ax2,'Fig3.2.Gray','Color','white');

               
                imshow(gauss,[],'Parent',ax3);
                title(ax3,'Fig3.3.Gaussian','Color','white');

                imshow(clahe,[],'Parent',ax4);
                title(ax4,'Fig3.4.CLAHE','Color','white');

                imshow(median_img,[],'Parent',ax5);
                title(ax5,'Fig3.5.Median','Color','white');

                imshow(edges,[],'Parent',ax6);
                title(ax6,'Fig3.6.Edge detection','Color','white');

                imshow(roi_edges,[],'Parent',ax7);
                title(ax7,'Fig3.7.ROI','Color','white');
            ...
            if mod(frameCount,2)==0
                ...
                title(ax8,'Fig3.8.Hough Line Detection',...
                    'Color','white');
            ...
            if ~isempty(left_lines) && ~isempty(right_lines)
                ...
                if abs(slopeL) < 0.3 || abs(slopeR) < 0.3
                    undetectedFrames = undetectedFrames + 1;
            ...
            imshow(line_frame,'Parent',ax9);
            title(ax9,'Fig3.9.Final','Color','white');
        ...
        % Detection Accuracy
        accuracy = (detectedFrames / frameCount) * 100;
        
        % False Detection Rate
        falseRate = (undetectedFrames / frameCount) * 100;
        
        % Average Processing Time
        avgProcessingTime = totalTime / frameCount;
        
        % Frames Per Second
        fps_value = frameCount / totalTime;
        
        %% Display Results in Command Window
        
        disp(' ');
        disp('======================================');
        disp(' QUANTITATIVE ANALYSIS RESULTS ');
        disp('======================================');
        ...
        fprintf('Undetected Frames      : %d\n', undetectedFrames);
        
        fprintf('Detection Accuracy     : %.2f %%\n', accuracy);
        
        fprintf('False Detection Rate   : %.2f %%\n', falseRate);
        
        fprintf('Average Processing Time: %.4f sec/frame\n', avgProcessingTime);
        
        fprintf('Average FPS            : %.2f FPS\n', fps_value);
